...
#define DEVICE_NAME                     "OSSTECH_UART"                               /**< Name of device. Will be included in the advertising data. */
...
static void adc_timer_handler(void * p_context)
{
  //ti ghi sau
    ...
    float volt = adc_val* 3.6 / 1024;
 
    memset(printf_buffer, 0x00, sizeof(printf_buffer));
    sprintf(printf_buffer, "Voltage: %f V \r\n",volt);
...
static void timers_init(void)
{
    ...
    //Create timer app
    err_code = app_timer_create(&adc_timer_id, APP_TIMER_MODE_REPEATED, adc_timer_handler);
    APP_ERROR_CHECK(err_code);
}
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    {
        uint32_t err_code;
        char temp_str[40];  // tao 1 mang luu chuoi ky tu

        NRF_LOG_DEBUG("Received data from BLE NUS. Writing data on UART.");
        NRF_LOG_HEXDUMP_DEBUG(p_evt->params.rx_data.p_data, p_evt->params.rx_data.length);

        memcpy(temp_str, p_evt->params.rx_data.p_data, p_evt->params.rx_data.length); // copy rx_data vao mang temp_str
        ...
        if( strcmp(temp_str, "led3on")==0) //bat led 
        {
            nrf_gpio_cfg_output(LED_3_pin19);
            nrf_gpio_pin_clear(LED_3_pin19);
        }

        if( strcmp(temp_str, "led3off")==0) //tat led
        {
            nrf_gpio_cfg_output(LED_3_pin19);
            nrf_gpio_pin_set(LED_3_pin19);
        }
        if( strcmp(temp_str, "led3toggle")==0) //tat led
        {
            nrf_gpio_cfg_output(LED_3_pin19);
            nrf_gpio_pin_toggle(LED_3_pin19);
        }
    }
...
int main(void)
{
    ...
    saadc_init();   
    uint32_t err_code = app_timer_start(adc_timer_id, ADC_INTERVAL, NULL);
